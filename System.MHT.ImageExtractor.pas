unit System.MHT.ImageExtractor;
...
type
  IMHTImageExtractor = interface(IInvokable)
    ['{BED708BD-93C3-4566-AF3A-50D1ED76AA14}']
    function FromFile(const Value: string): IMHTImageExtractor; overload;
    function FromFile(): string; overload;

    function ToPath(const Value: string): IMHTImageExtractor; overload;
    function ToPath(): string; overload;

    function Extract(): Boolean;
  end;

  TMHTImageExtractor = class(TInterfacedObject, IMHTImageExtractor)
  strict private
    FFromFile: string;
    FToPath: string;
    FExtractPath: string;
...
function TMHTImageExtractor.FromFile: string;
begin
  Result := FFromFile;
end;

function TMHTImageExtractor.ToPath(const Value: string): IMHTImageExtractor;
begin
  FToPath := Value;
  Result := Self;
end;

function TMHTImageExtractor.FromFile(const Value: string): IMHTImageExtractor;
begin
  FFromFile := Value;
  Result := Self;
end;

function TMHTImageExtractor.ToPath: string;
begin
  Result := FToPath;
end;

function TMHTImageExtractor.Extract: Boolean;
begin
...
procedure TMHTImageExtractor.ExtractMhtBoundary(const MhtContent: string);
var
  Content: string;
...
procedure TMHTImageExtractor.Extracting(BoundaryArray: TArray<string>);
var
  Idx: Integer;
begin
...
procedure TMHTImageExtractor.GetContentLocation(const BaseContent: string; ProcToSave: TProc<string, string>);
begin
...
procedure TMHTImageExtractor.ExtractImages(const Content: string);
begin
...
procedure TMHTImageExtractor.ExtractText(const Content: string);
begin
  GetContentLocation(Content,
...
//procedure TMHTImageExtractor.ExtractImages2(BoundaryArray: TArray<string>);
//var
//  Content: string;
//  IsImage: Boolean;
//  ImageName: string;
